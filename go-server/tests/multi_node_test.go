...
func TestMultiNodeNATSIntegration(t *testing.T) {
	brokerURL := os.Getenv("BLOCKFORGE_BROKER_URL")
	if brokerURL == "" {
		t.Skip("BLOCKFORGE_BROKER_URL not set, skipping multi-node NATS integration test")
	}
	...
	cfg2 := cfg1
	cfg2.NodeID = "node-integration-2"

	// Create Server 1
	s1, err := app.NewServer(cfg1, logger)
	if err != nil {
	...
	// Create Server 2
	s2, err := app.NewServer(cfg2, logger)
	if err != nil {
	...
	wsURL1 := "ws://" + l1.Addr().String() + "/ws?user_id=user-1"
	...
	wsURL2 := "ws://" + l2.Addr().String() + "/ws?user_id=user-2"
	...
	joinMsg1 := `{"version":"1","type":"room.join","request_id":"req-join-1","room_id":"room-multi"}`
	...
	joinMsg2 := `{"version":"1","type":"room.join","request_id":"req-join-2","room_id":"room-multi"}`
	...
	// Let the NATS subscription register
	time.Sleep(100 * time.Millisecond)

	// Broadcast from Client 1
	broadMsg := `{"version":"1","type":"room.broadcast","request_id":"req-broad-1","room_id":"room-multi","payload":{"text":"hello world"}}`
	...
	if receivedBroad.Payload.RoomID != "room-multi" {
		t.Errorf("expected room 'room-multi', got %q", receivedBroad.Payload.RoomID)
	}
	if receivedBroad.Payload.SenderID != "user-1" {
		t.Errorf("expected sender 'user-1', got %q", receivedBroad.Payload.SenderID)
	}

	// Send Private from Client 2 to Client 1
	privMsg := `{"version":"1","type":"private.send","request_id":"req-priv-1","recipient_id":"user-1","payload":{"text":"secret"}}`
	...
	if receivedPriv.Payload.SenderID != "user-2" {
		t.Errorf("expected sender 'user-2', got %q", receivedPriv.Payload.SenderID)
	}
	if receivedPriv.Payload.RecipientID != "user-1" {
		t.Errorf("expected recipient 'user-1', got %q", receivedPriv.Payload.RecipientID)
	}
}

func waitForReady(t *testing.T, s *app.Server) {
	t.Helper()
...
func expectAck(t *testing.T, c *websocket.Conn, reqID string) {
	...
	if ack.Type != "command.ack" {
		t.Fatalf("expected command.ack, got %q", ack.Type)
	}
	if ack.RequestID != reqID {
		t.Fatalf("expected request_id %q, got %q", reqID, ack.RequestID)
	}
}
